...
export class CameraManager {
    constructor(triggerFatalError, gui) {
    ...
    async setupCamerasAndStream() {
        try {
            ...
            if (mediaStreamTrack != null) {
                ...
                const wantedCamera = CameraAccess.adjustCamerasFromCameraStream(mediaStreamTrack, cameras, this.cameraType);
                if (wantedCamera == null) {
                    ...
                    if (mediaStreamTrack.readyState === "ended") {
                        return await this.reinitializeCamera();
                    }
                    return;
                }
            }
    ...
    getInitialResolutionFallbackLevel() {
        let resolutionFallbackLevel;
    ...
    async handleCameraInitializationError(camera, resolutionFallbackLevel, error) {
    ...
    async initializeCamera(camera, resolutionFallbackLevel) {
        if (camera == null) {
            throw new CustomError(CameraManager.noCameraErrorParameters);
        }
        this.stopStream();
        ...
        this.gui.setTorchTogglerVisible(false);
        resolutionFallbackLevel ??= this.getInitialResolutionFallbackLevel();
        try {
            ...
            return this.handleCameraInitializationError(camera, resolutionFallbackLevel, error);
        }
    }
